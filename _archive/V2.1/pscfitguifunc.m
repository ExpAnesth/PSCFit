...
%   fCutout = filtered version (for peak detection and plotting purposes)
%   fitHistCutout = event history 
%   d = raw data trace
%   dFit = fitted 
% Strucures:
...
persistent ds ap wp sp r tsl tsl_pts cutout fCutout fitHistCutout head d dFit
...
while ~done
  ...
  switch partJob
    case 'init'
      ...
      wp.noiseHiCFreq=500;
      % percentile of fit error beyond which events are automatically
      % kicked out
      wp.fitErrPrct=97;
      % --------------------------------------
...
%             'channels',{curDeblankChName});
          ...
          wp.si=si;
          % continuous trace in which fits of events will be placed
          dFit=zeros(size(d));
          % length of d
          wp.nd=numel(d);
    ...
    case 'genCutout'
      ...
      % polarity: if average of points in peak search interval is positive
      % we're dealing with positive-going events and vice versa
    ...
    case 'prepareFit'
      if ~isempty(cutout)
        ...
        % reset continuous trace of fit history
        dFit=zeros(size(d));
        % preallocation
        ...
        % iii) 10-90% rise time (determine from filtered version, too)
        ...
        r.t10=i1+ix(1)-1+wp.xOffs_pts;
        r.t90=i2+ix(1)-1+wp.xOffs_pts;
        ...
        plotProcessTrace(wp,r,cutout,fCutout,sp,tmpCoIx);
      else
        h=warndlg('no data loaded');
        uiwait(h);
        wp.isPrepareFit=false;
    ...
    case 'fit'
      ...
      if wp.isPrepareFit
        ...
        fitHistCutout=zeros(size(cutout));
        ...
        for g=1:wp.coNCol
          if r.OKIx(g)
            ...
            if ap.doPSCHistory && g>1 && r.qFit(g-1)>ap.minFitQuality
              ...
              % in fitHistCutout
              curHistCutout=dFit(tsl_pts(g)+wp.timeAx_pts);
              fitHistCutout(:,g)=curHistCutout;
              % 2. subtract it from present full cutout
              curCutout=cutout(:,g)-curHistCutout;
              % 3. re-adjust base line, keep the vaulue & cut out decay phase
            ...
            try
              ...
              % evaluate it for time interval stretching up to ten times
              % its decay time into the future and add to dFit. Do this
              % regardless of whether event history shall be used for event
              % fitting or not because dFit may in a future version be made
              % an output arg so the user can judge how well things worked.
              % Note that using radj as criterion favors large PSCs.
        ...
        tmpErrLim=prctile(r.qFit3,wp.fitErrPrct);
        tmpIx=find(r.qFit3>tmpErrLim);
        r.OKIx(tmpIx)=false;
        disp([int2str(numel(tmpIx)) ' PSCs marked for deletion based on fit error']);
        % consolidate r
        ...
        tmpCoIx=unique(tmpCoIx(2:end-1));
        plotFitTrace(ap,wp,r,cutout,fitHistCutout,sp,tmpCoIx);
    ...
    case 'reactCursorScatter'
      ...
      if ~isempty(wp.scatterD)
        ...
        plotFitTrace(ap,wp,r,cutout,fitHistCutout,sp,wp.curScatCoIx(1:min(20,numel(wp.curScatCoIx))));
        job(1)=[];
      else
        job(1)=[];
      end
    ...
    case 'deleteEvents'
      if wp.isPrepareFit
        ...
        if ~isempty(delIx)
          ...
          cutout(:,delIx)=[];
          fCutout(:,delIx)=[];
          if ~isempty(fitHistCutout), fitHistCutout(:,delIx)=[]; end
...
function plotProcessTrace(wp,r,cutout,fCutout,sp,coIx)
...
if ~isempty(coIx)
  ...
  [ylim,dy,yscaleFac,ph]=pllplot(fCutout(ix,coIx),'spacing','fixed',...
    'dy',dy,'ylim',ylim);
  ...
  for g=1:tmpN
    m1(g)=fCutout(r.t10(coIx(g))-wp.xOffs_pts,coIx(g));
    m2(g)=fCutout(r.t90(coIx(g))-wp.xOffs_pts,coIx(g));    
  end
...
function plotFitTrace(ap,wp,r,cutout,fitHistCutout,sp,coIx)
...
for g=coIx(:)'
  ...
  % fit
  % ph=plot(wp.timeAx_pts,fitHistCutout(:,g)+yOffs,'k');
  switch ap.nDecayComponent
    case 1
      ph=plot(wp.timeAx_pts(ix2),r.amp(1,g)*exp(-ix/r.tDecay(1,g))+yOffs+...
        fitHistCutout(ix2,g)+r.base(g));
    case 2
      ph=plot(wp.timeAx_pts(ix2),r.amp(1,g)*exp(-ix/r.tDecay(1,g))+...
        r.amp(2,g)*exp(-ix/r.tDecay(2,g))+yOffs+fitHistCutout(ix2,g)+r.base(g));
  end
