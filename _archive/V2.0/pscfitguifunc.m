...
%   sp=subplot handles
persistent ds ap wp sp tsl tsl_pts cutout fCutout head r d
...
while ~done
  ...
  switch partJob
    ...
    case {'clearData','readData'}
      ...
      ds.polarity=nan;
      ap.isAverageCutout=0;
      % this takes care of all fit results and plots except cutouts plot... 
      [wp,r]=discardResults(wp,r,sp);
      % ... done here
...
%             'channels',{curDeblankChName});
    ...
    case 'prepareFit'
      if ~isempty(cutout)
        ...
        % ii) polarity
        % if average of points in peak search interval is positive we're
        ...
        % iii) peaks, following Savitzky-Golay filtering (§§ could be a
    ...
    case 'fit'
      ...
      if wp.isPrepareFit
        ...
        % generate temporary hipass-filtered version of cutouts from which
        % rms of noise can be computed
        tmpCutout=hifi(cutout,wp.si,wp.noiseHiCFreq).^2;
        h=waitbar(0,'Fitting...');
        for g=1:wp.coNCol
          if r.OKIx(g)
            ...
            try
              if ap.nDecayComponent==1
                ...
                % to PSC's amplitude
                r.qFit3(g)=(gof.sse - sum(tmpCutout(ix2,g)))/numel(ix)/tmpPar(1);
              else
                ...
                % (as obtained via hipass filtering of traces), normalized
                % to PSC's amplitude
                r.qFit3(g)=(gof.sse - sum(tmpCutout(ix2,g)))/numel(ix)/tmpPar(3);
              end
            catch
        ...
        r.OKIx(tmpIx)=false;
        disp([int2str(numel(tmpIx)) ' PSCs deleted automatically based on fit error']);
        % consolidate r
    ...
    case 'deleteEvents'
      ...
      if ~isempty(delIx)
        ...
        cutout(:,delIx)=[];
        fCutout(:,delIx)=[];
        wp.coNCol=wp.coNCol-numel(delIx);
