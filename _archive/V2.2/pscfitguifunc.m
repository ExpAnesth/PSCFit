...
% to do:
% - interpolation of rising flank to get more precise estimates of rise
% time, particularly with undersampled data
...
%             'channels',{curDeblankChName});
    ...
    case 'prepareFit'
      if ~isempty(cutout)
        ...
        % all _pts parameters
        ix=(wp.xIntvPeak_pts(1):wp.xIntvPeak_pts(2))-wp.xOffs_pts;
        evr=evdeal(cutoutFilt(ix,:),'idx',{'minmaxpeak'});
        r.aPeak=evr.maxPeak;
        ...
        [m,i1]=max(cutoutFilt(ix,:)>=repmat(.1*r.aPeak,numel(ix),1));
        [m,i2]=max(cutoutFilt(ix,:)>=repmat(.9*r.aPeak,numel(ix),1));
        r.tRise=i2-i1;
...
if ~isempty(coIx)
  ...
  for g=1:tmpN
    m1(g)=cutoutFilt(r.t10(coIx(g))-wp.xOffs_pts,coIx(g));
    m2(g)=cutoutFilt(r.t90(coIx(g))-wp.xOffs_pts,coIx(g));    
  end
